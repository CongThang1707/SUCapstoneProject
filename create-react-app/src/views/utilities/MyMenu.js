...
const MyMenu = () => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      const brandId = localStorage.getItem('brandId');
      try {
        const response = await axios.get(`https://3.1.81.96/api/Categories?brandId=${brandId}`);
      ...
      <Dialog open={showAddMenuDialog} onClose={handleCloseAddMenuDialog}>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth variant="standard" sx={{ mt: 2 }}>
            <InputLabel id="category-select-label">Menu Name</InputLabel>
            <Select
              labelId="category-select-label"
              id="category-select"
              name="menuName" // Corrected name to match with handleAddMenuChange
              value={newMenuData.menuName}
              onChange={handleMenuNameChange} // Update the function to handleMenuNameChange
              label="Menu Name"
            >
